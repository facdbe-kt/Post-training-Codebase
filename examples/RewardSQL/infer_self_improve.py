...
from sql_decoders import SQLSelfImprove
...
def inference(test_set, dbs, args):
    ...
    for index, item in enumerate(tqdm(test_set)):
        ...
        if index < 2:
            ...
            print(f"初始SQL: {metadata['initial_result']}")
            print(f"最终SQL: {sql}")
            print(f"迭代次数: {metadata['iterations']}")
            print(f"改进历史: {metadata['improvement_history']}")
            print()
        ...
        results.append({
            ...
            "initial_answer": metadata["initial_result"],
            "iterations": metadata["iterations"],
            "improvement_history": metadata["improvement_history"]
        })
    ...
    # 保存完整结果（包括改进历史）
    with open(os.path.join(args.output, "improvement_history.json"), "w") as file:
        json.dump(results, file, indent=4)
    
    return results

def main():
    parser = argparse.ArgumentParser(description="使用自我改进解码策略进行SQL推理")
    ...
    parser.add_argument("--max_iterations", type=int, default=2, help="最大自我改进迭代次数")
